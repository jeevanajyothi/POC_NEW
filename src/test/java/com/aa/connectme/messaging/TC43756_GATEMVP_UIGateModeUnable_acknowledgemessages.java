...
/******************************************************************************************************************************************************
 * 
 * Short Description :: Test To Verify Default time range flights are displayed successfully.
 *                       
 * ****************************************************************************************************************************************************/
...
public class TC43756_GATEMVP_UIGateModeUnable_acknowledgemessages extends UtilityFunction {
	...
	utilityFunction.DateAndTime = currentDateAndTime();
		...
		for (int i=1; i<2; i++)
		{
			...
			//driver = utilityFunction.initilizeDriver(BrowserName,Environment);
			System.out.println("*****TC43756_GATEMVP_UIGateModeUnable_acknowledgemessages******");	
			System.out.println("---------------------------------------------------------------");
			...
			try {
				...
				/*if (MP.verifyunableAcknowledge() == true){
					System.out.println("We can't Acknowledge the messages when we are in gate mode.");
					ExtentTest childTest1 = parentTest.createNode("STEP 3 - Acknowledge messages ", "AAcknowledge messages").pass("We can't Acknowledge the messages when we are in gate mode.");
					res = true;
				}else {
					System.out.println("It is allowing us to acknowledge the messages in the gate mode.");
					report.Reporter_ReportEvent(driver, "Messaging panel verified", Constants.EXTENT_REPORT_FAIL,
							"Messaging panel not verified");
					ExtentTest childTest1 = parentTest.createNode("STEP 3 - Acknowledge messages", "Acknowledge messages").fail("We are able to acknowledge the messages when we are in the gate mode.");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
				}*/
