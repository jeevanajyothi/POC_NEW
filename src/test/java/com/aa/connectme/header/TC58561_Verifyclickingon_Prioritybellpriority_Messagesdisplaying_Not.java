...
public class TC58561_Verifyclickingon_Prioritybellpriority_Messagesdisplaying_Not extends UtilityFunction {
	...
	public void Test(String BrowserName, String Environment, String TestResult) throws Exception {
		...
		for (int i = 1; i < 2; i++) {
			...
			System.out.println("*****TC58561_Verifyclickingon_Prioritybellpriority_Messagesdisplaying_Not******");
			System.out.println("-------------------------------------------------------------------------------");
			try {
				...
				if(dashboardPage.verifyPriorityBell()==true)
				{
					...
					ExtentTest childTest1 = parentTest.createNode("STEP 8 - Verify Priority bell ", "Priority bell presense in dashboard page").pass("Priority bell displayed in dashboard page");
					res = true;
				}else{
					System.out.println("Priority bell not displayed in dashboard page");
					ExtentTest childTest1 = parentTest.createNode("STEP 8 - Verify Priority bell", "Priority bell presense in dashboard page").fail("Priority bell  not displayed in dashboard page");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
				}
				...
				if(dashboardPage.clickOnPriorityBell()==true)
				{
					...
					ExtentTest childTest1 = parentTest.createNode("STEP 9 - Click on priority bell", "Clicked on priority bell ").pass("Clicked on priority bell suceesfully");
					res = true;
				}else{
					System.out.println("Failed to click on priority bell");
					ExtentTest childTest1 = parentTest.createNode("STEP 9 - Click on priority bell", "Clicked on priority bell ").fail("");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
				}
				...
				if(dashboardPage.getListOfMesFromNotificationTab()==true)
				{
					...
					ExtentTest childTest1 = parentTest.createNode("STEP 10 - Acknowledge messages", "Priority message count matches").pass("Priority message count matches with unacknowkedged priority messages");
					res = true;
				}else{
					System.out.println("Priority message count not matches with unacknowkedged priority messages");
					ExtentTest childTest1 = parentTest.createNode("STEP 10 - Acknowledge messages", "Priority message count matches").fail("Priority message count not matches with unacknowkedged priority messages");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
				}
