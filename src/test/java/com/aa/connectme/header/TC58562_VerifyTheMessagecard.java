...
public class TC58562_VerifyTheMessagecard extends UtilityFunction {
	...
	public void Test(String BrowserName, String Environment, String TestResult) throws Exception {
		...
		for (int i = 1; i < 2; i++) {
			...
			System.out.println("*****TC58562_VerifyTheMessagecard_Bellnotification_preview_automatedmsgs_MessageBody******");
			System.out.println("-------------------------------------------------------------");
			try {
				...
				if (dashboardPage.getMesTitleFromNotificationTab() == true) {
					...
					ExtentTest childTest1 = parentTest
							.createNode("STEP 11 - Get message body", "Notification message with messagebody displayed")
							.pass("Automated messages displayed with MessageBody");
					res = true;
				} else {
					System.out.println("Automated messages are not displayed");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 11 - Get message body",
