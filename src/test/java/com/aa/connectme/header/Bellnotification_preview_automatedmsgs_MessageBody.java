...
public class Bellnotification_preview_automatedmsgs_MessageBody extends UtilityFunction {
	...
	public void Test(String BrowserName, String Environment, String TestResult) throws Exception {
		...
		for (int i = 1; i < 2; i++) {
			...
			System.out.println("*****Bellnotification_preview_automatedmsgs_MessageBody******");
			System.out.println("-------------------------------------------------------------");
			try {
				...
				if (dashboardPage.verifyPriorityBell() == true) {
					...
					ExtentTest childTest1 = parentTest
							.createNode("STEP 9 - Verify Priority bell ", "Priority bell presense in dashboard page")
				...
				if (dashboardPage.clickOnPriorityBell() == true) {
					...
					ExtentTest childTest1 = parentTest
							.createNode("STEP 10 - Click on priority bell", "Clicked on priority bell ")
							.pass("Clicked on priority bell suceesfully");
					res = true;
				} else {
					System.out.println("Failed to click on priority bell");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 10 - Click on priority bell", "Clicked on priority bell ").fail("");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
				}
				...
				if (dashboardPage.getListOfMesFromNotificationTab() == true) {
					...
					ExtentTest childTest1 = parentTest
							.createNode("STEP 11 - Acknowledge messages", "Priority message count matches")
				...
				if (dashboardPage.getMesTitleFromNotificationTab() == true) {
					System.out.println("Automated messages displayed with MessageBody\r\n");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 12 - Get message body", "Notification message with messagebody displayed")
							.pass("Automated messages displayed with MessageBody");
					res = true;
				} else {
					System.out.println("Automated messages are not displayed");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 12 - Get message body",
									"Notification message with messagebody not displayed")
							.fail("Automated messages not displayed with MessageBody");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
