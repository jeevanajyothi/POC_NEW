...
public class Bellnotification_preview_automatedmsgs_Gateflight_Topics extends UtilityFunction {
	...
	public void Test(String BrowserName, String Environment, String TestResult) throws Exception {
		...
		for (int i = 1; i < 2; i++) {
			...
			System.out.println("*****Bellnotification_preview_automatedmsgs_Gateflight_Topics******");
			System.out.println("-------------------------------------------------------------------");
			try {
				...
				if (logoutPage.logout() == true) {
					...
					ExtentTest childTest1 = parentTest
							.createNode("STEP 7 - Personal logout ", "Personal logout from the application")
				...
				if (loginPage.Personnelogin(UserID, Password) == true) {
					System.out.println("Login succesfull");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 8 - Personal login ", "Personal login with user credentials")
							.pass("Personal login successful");
					res = true;
				} else {
					System.out.println("Login Unsuccesfull");
				...
				if (dashboardPage.verifyPriorityBell() == true) {
					...
					ExtentTest childTest1 = parentTest
							.createNode("STEP 10 - Verify Priority bell ", "Priority bell presense in dashboard page")
				...
				if (dashboardPage.clickOnPriorityBell() == true) {
					...
					ExtentTest childTest1 = parentTest
							.createNode("STEP 11 - Click on priority bell", "Clicked on priority bell ")
							.pass("Clicked on priority bell suceesfully");
					res = true;
				} else {
					System.out.println("Failed to click on priority bell");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 11 - Click on priority bell", "Clicked on priority bell ").fail("");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
				}
				...
				if (dashboardPage.getListOfMesFromNotificationTab() == true) {
					...
					ExtentTest childTest1 = parentTest
							.createNode("STEP 12 - Acknowledge messages", "Priority message count matches")
				...
				if (dashboardPage.getMesTopicFromNotificationTab() == true) {
					System.out.println("Automated messages displayed with Gate or Flight topic");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 13 - Get message topic",
									"Acknowledging all the messages by clicking on Acknowledge all tick mark")
							.pass("Automated messages displayed with Gate or Flight topic");
					res = true;
				} else {
					System.out.println("Automated messages are not displayed");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 13 - Get message topic", "There were no messages as of now.")
							.fail("Automated messages not displayed with Gate or Flight topic");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
