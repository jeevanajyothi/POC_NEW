...
public class Bellnotification_preview_automatedmsgs_Roles extends UtilityFunction{
    WebDriver driver;
	...
	public void Test(String BrowserName, String Environment, String TestResult) throws Exception {
		...
		for (int i = 1; i < 2; i++) {
			...
			System.out.println("*****Bellnotification_preview_automatedmsgs_Roles******");
			System.out.println("-------------------------------------------------------");
			try {
				...
				if (dashboardPage.getRolesFromNotificationTab() == true) {
					System.out.println("Automated messages displayed with Roles");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 12 - Get Roles",
									"Notification message with Roless displayed")
							.pass("Automated messages displayed with Roles");
					res = true;
				} else {
					System.out.println("Automated messages are not displayed");
					ExtentTest childTest1 = parentTest
							.createNode("STEP 12 - GetRoles", "Notification message with Roless displayed")
							.fail("There were no Automated messages displayed with roles time");
					str = utilityFunction.captureScreenshot();
					childTest1.addScreenCaptureFromPath(str);
					res = false;
